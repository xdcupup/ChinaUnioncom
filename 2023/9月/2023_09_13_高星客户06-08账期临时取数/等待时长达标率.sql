select "FWBZ032"                                                        as index_code,
       ...
       case when numerator / denominator >= 0.9 then "是" else "否" end as reach_standard,
       "{0}"                                                            as month_id,
       ""                                                               as day_id,
...
from (select province_name,
             sum(cast(nvl(wait_time_10, "0") as int) + cast(nvl(wait_time_5, "0") as int) +
                 cast(nvl(wait_time_4, "0") as int))    as numerator,
             sum(cast(nvl(total_num_take, "0") as int)) as denominator
      from dc_dwa_cbss.DWA_D_MRT_E_TWO_TIME_LENGTH_REPORT
      where month_id = "202306"
      group by province_name) t
         left join (select distinct prov_code, prov_name from pc_dwd.cem_dim_pro_city_regoin) code
                   on t.province_name = code.prov_name