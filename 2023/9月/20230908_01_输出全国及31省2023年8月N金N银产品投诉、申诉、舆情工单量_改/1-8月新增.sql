...
create table dc_dwd.lz_gold_silver_20230108 as
select
sheet_id                         ,
sheet_no                         ,
sheet_type                       ,
...
case when sheet_type='01' then compl_prov_name
     else busno_prov_name end sheet_pro_name,    -- 工单归属省分名称
case when sheet_type='01' then compl_prov
     else busno_prov_id end sheet_pro,           -- 工单归属省分编码
case when sheet_type='01' then compl_area_name
     else cust_city_name end  sheet_area_name,   -- 工单归属地市名称
case when sheet_type='01' then compl_area
...
is_shensu,                                  --是否申诉工单
month_id,
day_id from dc_dwa.dwa_d_sheet_main_history_chinese
 where month_id in('202301','202302','202303','202304','202305','202306','202307','202308')
 and (
 regexp(upper(serv_content),'AI秘书|XR通话|数字人|畅视多视频|.*哔哩.*定向流量包.*|.*芒果.*定向流量包.*|.*腾讯.*定向流量包.*|.*优酷.*定向流量包.*|.*爱奇艺.*定向流量包.*|.*阿里.*定向流量包.*|.*百度.*定向流量包.*|.*抖音.*定向流量包.*|.*定向流量.*5G加油包.*|数字人|沃视频|5G宽视界|手机视频|家庭会员|联通学堂|.*沃音乐.*精英.*|.*沃音乐.*家庭.*|.*沃音乐.*成长.*|联通畅游|沃畅游|联通爱听|加速宝|视频客服|视频会议|AI翻译')=true
)
and is_delete='0' -- 不统计逻辑删除工单
and sheet_status not in ('11','8') -- 不统计废弃工单
and (
( -- 公众
accept_channel not in ('08','09','14','15','19','20','22','25','82','83','84','85','86','87','88','89','90','91','92','152','151','18') -- 剔除掉指定的受理渠道
...
;
select count(*) from dc_dwd.lz_gold_silver_20230108;
...
select sheet_pro_name,
       ...
       month_id,
       '投诉'
from dc_dwd.lz_gold_silver_20230108 tb1
...
where tb1.accept_channel = '13' and tb1.submit_channel= '05' and tb1.pro_id = 'AA'
and nvl(sheet_pro,'')!=''
