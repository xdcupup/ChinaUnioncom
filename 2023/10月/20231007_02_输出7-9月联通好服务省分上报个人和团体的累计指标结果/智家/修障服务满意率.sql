...
-- 建表
drop table dc_dwd.zhijia_xzfw_person_base_detail_1007_temp;
create table dc_dwd.zhijia_xzfw_person_base_detail_1007_temp as
...
-- 插入数据
INSERT overwrite table dc_dwd.zhijia_xzfw_person_base_detail_1007_temp
select phone,sheet_code,accept_time,create_time,one_answer
 from dc_dwd.dwd_d_nps_satisfac_details_broadband_repair
where month_id in ('202307','202308','202309');
select count(*) from dc_dwd.zhijia_xzfw_person_base_detail_1007_temp;

-- todo 创建用于计算的修障服务满意率个人表
-- 建表
drop table dc_dwd.dwd_d_evt_zj_xzfw_person_calculate_detail_20231007;
create table dc_dwd.dwd_d_evt_zj_xzfw_person_calculate_detail_20231007 as
...
from (
   ...
   left join dc_dwd.zhijia_xzfw_person_base_detail_1007_temp d_t on t_a.device_number=d_t.phone
   union all
   select
     t_a.prov_name,
     t_a.xm,
     ...
     d_t.phone,d_t.sheet_code,d_t.accept_time,d_t.create_time,d_t.one_answer
   from dc_dwd.zhijia_personal_1007 t_a
   left join dc_dwd.zhijia_xzfw_person_base_detail_1007_temp d_t where  locate(t_a.xz_order,d_t.sheet_code)>0 and t_a.xz_order    is not null and t_a.xz_order!=''
) A where 1>2;
-- 插入数据
INSERT overwrite table dc_dwd.dwd_d_evt_zj_xzfw_person_calculate_detail_20231007
select *,ROW_NUMBER() OVER(PARTITION BY staff_code,device_number,xz_order,phone,sheet_code ORDER BY create_time desc) S_RN
from (
   select
     t_a.prov_name,
...
select * from dc_dwd.dwd_d_evt_zj_xzfw_person_calculate_detail_20231007;
-- todo 个人结果
set hive.mapred.mode = unstrict;
SET mapreduce.job.queuename=q_dc_dw;
select  prov_name,
...
select   (case when one_answer is not null and one_answer rlike '10' then 1 else 0 end)  as fenzi,
    ...
    (one_answer rlike '10' or one_answer rlike '7' or one_answer rlike '3') then 1 else 0 end) as fenmu,
        ...
        d.xm,
        d.staff_code
        from dc_dwd.dwd_d_evt_zj_xzfw_person_calculate_detail_20231007 d where S_RN=1
...
set hive.mapred.mode = unstrict;
SET mapreduce.job.queuename=q_dc_dw;
drop table dc_dwd.dwd_d_evt_zj_xzfw_team_calculate_detail_20231007;
-- 建表
create table dc_dwd.dwd_d_evt_zj_xzfw_team_calculate_detail_20231007 as
select *,ROW_NUMBER() OVER(PARTITION BY staff_code,device_number,xz_order,phone,sheet_code ORDER BY create_time desc) S_RN
from (
   select
     t_a.prov_name,
...
-- 注入数据
insert overwrite table dc_dwd.dwd_d_evt_zj_xzfw_team_calculate_detail_20231007
select *,ROW_NUMBER() OVER(PARTITION BY staff_code,device_number,xz_order,phone,sheet_code ORDER BY create_time desc) S_RN
from (
   select
     t_a.prov_name,
...
select   (case when one_answer is not null and one_answer rlike '10' then 1 else 0 end)  as fenzi,
    ...
    (one_answer rlike '10' or one_answer rlike '7' or one_answer rlike '3') then 1 else 0 end) as fenmu,
        ...
        d.xm,
        d.staff_code
        from dc_dwd.dwd_d_evt_zj_xzfw_team_calculate_detail_20231007 d where S_RN=1
