...
drop table dc_dwd.zhijia_person_clcs_base_detail_1007_temp;
create table dc_dwd.zhijia_person_clcs_base_detail_1007_temp as
    select
 ...
 from (select *, row_number() over (partition by id order by dts_kaf_offset desc) rn
      ...
      where province_name is not null
        and month_id in  ('202307','202308','202309')
        and one_answer not like '%4%'
        and one_answer != '')  a
where a.rn = '1' and 1>2;

insert overwrite table dc_dwd.zhijia_person_clcs_base_detail_1007_temp
select
    a.sheet_code,a.two_answer,accept_time,create_time,phone
 from (select *, row_number() over (partition by id order by dts_kaf_offset desc) rn
      from dc_dwd.dwd_d_nps_satisfac_details_repair_new
      where province_name is not null
        ...
        and one_answer != '')  a
where a.rn = '1';
select * from dc_dwd.zhijia_person_clcs_base_detail_1007_temp;

-- todo 创建用于计算的存量测速满意率个人表
-- 建表
drop table dc_dwd.dwd_d_evt_zj_clcs_person_calculate_detail_20231007;
create table dc_dwd.dwd_d_evt_zj_clcs_person_calculate_detail_20231007 as
...
from (
   ...
   left join dc_dwd.zhijia_person_clcs_base_detail_1007_temp d_t on t_a.device_number=d_t.phone
   union all
   select
     t_a.prov_name,
     t_a.xm,
     ...
     d_t.phone,d_t.two_answer,d_t.accept_time,d_t.sheet_code,d_t.create_time
   from dc_dwd.zhijia_personal_1007 t_a
   left join dc_dwd.zhijia_person_clcs_base_detail_1007_temp d_t where locate(t_a.xz_order,d_t.sheet_code)>0 and t_a.xz_order    is not null and t_a.xz_order!=''
) A where 1>2;
-- 插入数据
insert overwrite table dc_dwd.dwd_d_evt_zj_clcs_person_calculate_detail_20231007
select *,ROW_NUMBER() OVER(PARTITION BY staff_code,device_number,xz_order,phone,sheet_code ORDER BY create_time desc) S_RN
from (
   select
     t_a.prov_name,
...
select * from dc_dwd.dwd_d_evt_zj_clcs_person_calculate_detail_20231007;
-- todo 个人结果
select  prov_name,
       '智家工程师'       window_type,
       xm,
...
select  case when two_answer is not null and two_answer rlike '1'  then 1 else 0 end  as fenzi,
        ...
        d.xm,
        d.staff_code
        from dc_dwd.dwd_d_evt_zj_clcs_person_calculate_detail_20231007 d where S_RN=1
...
-- 建表
drop table dc_dwd.dwd_d_evt_zj_clcs_team_calculate_detail_20231007;
create table dc_dwd.dwd_d_evt_zj_clcs_team_calculate_detail_20231007 as
select *,ROW_NUMBER() OVER(PARTITION BY staff_code,device_number,xz_order,phone,sheet_code ORDER BY create_time desc) S_RN
from (
   select
     t_a.prov_name,
...
-- 注入数据
insert overwrite table dc_dwd.dwd_d_evt_zj_clcs_team_calculate_detail_20231007
select *,ROW_NUMBER() OVER(PARTITION BY staff_code,device_number,xz_order,phone,sheet_code ORDER BY create_time desc) S_RN
from (
   select
     t_a.prov_name,
...
select distinct xm from dc_dwd.dwd_d_evt_zj_clcs_team_calculate_detail_20231007;
...
select  case when two_answer is not null and two_answer rlike '1'  then 1 else 0 end  as fenzi,
        ...
        d.xm,
        d.staff_code
        from dc_dwd.dwd_d_evt_zj_clcs_team_calculate_detail_20231007 d where S_RN=1
