...
from dc_dwd_cbss.dwd_d_evt_cb_trade_his c
      where
        ...
        and regexp_replace(c.finish_date,'-','') like '202308%' and 1>2 ;

insert overwrite table dc_dwd.dpa_trade_all_dist_temp_08
select
device_number serial_number,
substr(prov_id,2,2) province_code,
accept_date,
...
select count(distinct *) from dc_dwd.dpa_trade_all_dist_temp_08;



-----
create table dc_dwd.dpa_trade_all_dist_temp_08_school as
    select c.serial_number, c.province_code, c.accept_date, c.finish_date
        from dc_dwd.dpa_trade_all_dist_temp_08 c  join  dc_dwd.product_dim_temp_1031 p
             on c.product_id = p.product_id
        where p.product_name like '%校园%' or p.product_name like '%沃派%' or p.product_name like '%学生%';
-----
insert overwrite table dc_dwd.dpa_trade_all_dist_temp_08_school
    select c.serial_number, c.province_code, c.accept_date, c.finish_date
        from dc_dwd.dpa_trade_all_dist_temp_08 c  join  dc_dwd.product_dim_temp_1031 p
             on c.product_id = p.product_id
        where p.product_name like '%校园%' or p.product_name like '%沃派%' or p.product_name like '%学生%';
...
create table dc_dwd.service_standardization_temp_08 as
...
from (select province_code,
             if((from_unixtime(unix_timestamp(accept_date, 'yyyyMMddHHmmss'), 'H') < 16 and
                ...
                0) as zhuangji_good,
             '1'   as zhuangji_total
      from dc_dwd.dpa_trade_all_dist_temp_08_school ) tt
group by tt.province_code;



insert overwrite table dc_dwd.service_standardization_temp_08
select tt.province_code as pro_id, sum(zhuangji_good) as fenzi, count(zhuangji_total) as fenmu
from (select province_code,
             if((from_unixtime(unix_timestamp(accept_date, 'yyyyMMddHHmmss'), 'H') < 16 and
                 (from_unixtime(unix_timestamp(finish_date, 'yyyyMMddHHmmss'), 'd') -
...
select
    ...
    a.fenzi numerator,
    '202308' as zhangqi
from dc_dwd.service_standardization_temp_08 a  left join dc_dim.dim_province_code b on a.pro_id = b.code ;
