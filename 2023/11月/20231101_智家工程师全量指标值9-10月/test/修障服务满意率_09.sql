...
-----------------9月----------------
-- 建表
drop table dc_dwd.zhijia_xzfw_person_base_detail_1101_09_temp;
create table dc_dwd.zhijia_xzfw_person_base_detail_1101_09_temp as
...
-- 插入数据
insert overwrite table dc_dwd.zhijia_xzfw_person_base_detail_1101_09_temp
select phone, sheet_code, accept_time, create_time, one_answer
from dc_dwd.dwd_d_nps_satisfac_details_broadband_repair
where month_id in ('202309');
select count(*)
from dc_dwd.zhijia_xzfw_person_base_detail_1101_09_temp;

-- todo 创建用于计算的修障服务满意率个人表
-- 建表
drop table dc_dwd.dwd_d_evt_zj_xzfw_person_calculate_detail_20231101_09;
create table dc_dwd.dwd_d_evt_zj_xzfw_person_calculate_detail_20231101_09 as
select *,
...
from (select t_a.prov_name,
      ...
      from dc_dwd.zhijia_info_temp09 t_a
               left join dc_dwd.zhijia_xzfw_person_base_detail_1101_09_temp d_t on t_a.device_number = d_t.phone
      union all
      select t_a.prov_name,
             ...
             d_t.one_answer
      from dc_dwd.zhijia_info_temp09 t_a
               left join dc_dwd.zhijia_xzfw_person_base_detail_1101_09_temp d_t
...
insert overwrite table dc_dwd.dwd_d_evt_zj_xzfw_person_calculate_detail_20231101_09
select *,
       row_number() over (partition by staff_code,device_number,xz_order,phone,sheet_code order by create_time desc) S_RN
from (select t_a.prov_name,
             t_a.city,
...
select *
from dc_dwd.dwd_d_evt_zj_xzfw_person_calculate_detail_20231101_09
where one_answer is not null;
select count(*)
from dc_dwd.dwd_d_evt_zj_xzfw_person_calculate_detail_20231101_09
where one_answer is not null;
-- todo 个人结果
...
from (select o_a.prov_name,
      ...
      from (select prov_name, city, county, banzu, staff_code, xm from dc_dwd.zhijia_info_temp_09) o_a
               left join (select (case when one_answer is not null and one_answer rlike '10' then 1 else 0 end) as fenzi,
                                 ...
                                 d.xm,
                                 d.staff_code
                          from dc_dwd.dwd_d_evt_zj_xzfw_person_calculate_detail_20231101_09 d
