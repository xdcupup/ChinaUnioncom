...
create table if not exists dc_dwd.dwd_d_evt_cb_trade_his_09 as
select device_number,trade_id,accept_date,finish_date from dc_dwd_cbss.dwd_d_evt_cb_trade_his where 1 > 2;


select device_number,trade_id,accept_date,finish_date from dc_dwd_cbss.dwd_d_evt_cb_trade_his limit 10;
...
drop table dc_dwd.dwd_d_evt_zj_drt_person_detail_112_09;
...
select count(*) from dc_dwd.dwd_d_evt_zj_drt_person_detail_112_09 where S_RN =1 and  trade_id is not null ;


select count(*) from dc_dwd.dwd_d_evt_zj_drt_person_detail_112_09;
...
from (
select prov_name, city, county, banzu, staff_code, xm from dc_dwd.zhijia_info_temp09
) o_a
left join (
select if((from_unixtime(unix_timestamp(accept_date, 'yyyyMMddHHmmss'), 'H') < 16 and
       ...
       case when trade_id is not null  then 1  else 0 end as zhuangji_total,
        ...
        d.xm,
        d.staff_code
        from dc_dwd.dwd_d_evt_zj_drt_person_detail_112_09 d where S_RN=1
) o_b on o_a.staff_code=o_b.staff_code
) o_o_a
group by  o_o_a.staff_code, o_o_a.xm, o_o_a.prov_name, o_o_a.city, o_o_a.county, o_o_a.banzu;">zhijia09_2.csv