...
drop table dc_dwd.dwd_d_evt_zj_xz_detail_20231101_09_temp;
------todo 1.1 建表 修障当日好率明细
create table dc_dwd.dwd_d_evt_zj_xz_detail_20231101_09_temp as
select a.archived_time,a.accept_time,a.sheet_no,a.busi_no
      ...
      where a.month_id in ('202309')
        ...
        and a.serv_type_name in ('故障工单（2021版）>>融合>>【网络使用】宽带网络>>宽带无法上网',
                                ...
                                and 1>2;
--- 9月
INSERT overwrite table dc_dwd.dwd_d_evt_zj_xz_detail_20231101_09_temp
select a.archived_time,a.accept_time,a.sheet_no,a.busi_no
      from dc_dwa.dwa_d_sheet_main_history_chinese a
      where a.month_id in ('202309')
        and a.is_delete = '0'
...
select * from dc_dwd.dwd_d_evt_zj_xz_detail_20231101_09_temp;

-- todo 创建用于计算的修障当日好个人表
drop table dc_dwd.dwd_d_evt_xzdrh_person_detail_20231101_09;
create table dc_dwd.dwd_d_evt_xzdrh_person_detail_20231101_09 as
...
from (
   ...
   left join dc_dwd.dwd_d_evt_zj_xz_detail_20231101_09_temp d_t on t_a.device_number=d_t.busi_no
   union all
   select
     t_a.prov_name,
     t_a.city,
     ...
     d_t.sheet_no,d_t.busi_no,d_t.accept_time,d_t.archived_time
   from dc_dwd.zhijia_info_temp09 t_a
   left join dc_dwd.dwd_d_evt_zj_xz_detail_20231101_09_temp d_t  where locate(t_a.xz_order,d_t.sheet_no)>0 and t_a.xz_order is not null and t_a.xz_order!=''
) A where 1>2;
-- todo 插入数据
insert overwrite table dc_dwd.dwd_d_evt_xzdrh_person_detail_20231101_09
select *,ROW_NUMBER() OVER(PARTITION BY staff_code,sheet_no,busi_no ORDER BY accept_time desc) S_RN
from (
   select
     t_a.prov_name,
...
SELECT * from dc_dwd.dwd_d_evt_xzdrh_person_detail_20231101_09;

SELECT count(*) from dc_dwd.dwd_d_evt_xzdrh_person_detail_20231101_09;
...
select if((substring(d.accept_time, 12, 2) < 16 and
       ...
       case when sheet_no is not null  then 1  else 0 end as fenmu,
        ...
        d.xm,
        d.staff_code
        from dc_dwd.dwd_d_evt_xzdrh_person_detail_20231101_09 d where S_RN=1
