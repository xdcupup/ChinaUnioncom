...
drop table dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp_js;
------todo 1.1 建表 修障当日好率明细
create table dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp_js as
select a.archived_time,a.accept_time,a.sheet_no,a.busi_no
      ...
      where a.month_id in ('202310')
        ...
        and a.serv_type_name in ('故障工单（2021版）>>融合>>【网络使用】宽带网络>>宽带无法上网',
                                ...
                                and 1>2;
--- 10月
INSERT overwrite table dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp_js
select a.archived_time,a.accept_time,a.sheet_no,a.busi_no
      from dc_dwa.dwa_d_sheet_main_history_chinese a
      where a.month_id in ('202310')
        and cust_area_name = '江苏'
        and a.is_delete = '0'
        and a.sheet_type = '04'
        and (pro_id NOT IN ("S1", "S2", "N1", "N2")
          OR (pro_id IN ("S1", "S2", "N1", "N2")
...
select * from dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp_js;
...
-- drop table dc_dwd.dwd_d_evt_xzdrh_person_detail_20231101_10;
-- create table dc_dwd.dwd_d_evt_xzdrh_person_detail_20231101_10 as
...
--    left join dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp_js d_t on t_a.device_number=d_t.busi_no
--    union all
--    select
--      t_a.prov_name,
--      t_a.city,
...
--      d_t.sheet_no,d_t.busi_no,d_t.accept_time,d_t.archived_time
--    from dc_dwd.zhijia_info_temp10 t_a
--    left join dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp_js d_t  where locate(t_a.xz_order,d_t.sheet_no)>0 and t_a.xz_order is not null and t_a.xz_order!=''
-- ) A where 1>2;
-- -- todo 插入数据
-- insert overwrite table dc_dwd.dwd_d_evt_xzdrh_person_detail_20231101_10
-- select *,ROW_NUMBER() OVER(PARTITION BY staff_code,sheet_no,busi_no ORDER BY accept_time desc) S_RN
-- from (
--    select
--      t_a.prov_name,
...
--      d_t.sheet_no,d_t.busi_no,d_t.accept_time,d_t.archived_time
--    from dc_dwd.zhijia_info_temp10 t_a
--    left join dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp_js d_t  on t_a.xz_order=d_t.sheet_no and t_a.xz_order is not null and t_a.xz_order!=''
-- ) A;
-- SELECT * from dc_dwd.dwd_d_evt_xzdrh_person_detail_20231101_10;
--
-- SELECT count(*) from dc_dwd.dwd_d_evt_xzdrh_person_detail_20231101_10;
...
-- from (
--     select prov_name,city,county,banzu,staff_code,xm from dc_dwd.zhijia_info_temp10
-- ) o_a
-- left join (
...
--         d.xm,
--         d.staff_code
--         from dc_dwd.dwd_d_evt_xzdrh_person_detail_20231101_10 d where S_RN=1
...
-- 统计
select prov_name,city,county,banzu,xm,staff_code , sum(fenzi) as fenzi,sum(fenmu) as fenmu,sum(fenzi)/sum(fenmu) as zhbiaozhi from (
select d_t.sheet_no,
       d_t.archived_time,
       d_t.accept_time,
       t_a.prov_name,
    ...
    if((substring(accept_time, 12, 2) < 16 and
                 archived_time < concat(substring(date_add(accept_time, 0), 1, 10), ' ', '23:59:59'))
                    or (substring(accept_time, 12, 2) >= 16 and
                        ...
                        and sheet_no is not null), 1, 0) as fenzi,
       case when sheet_no is not null  then 1  else 0 end as fenmu
from dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp_js d_t left join dc_dwd.zhijai_temp_js10 t_a on t_a.xz_order=d_t.sheet_no)a group by prov_name, city, county, banzu, xm, staff_code;
    ...
    if((substring(accept_time, 12, 2) < 16 and
                 archived_time < concat(substring(date_add(accept_time, 0), 1, 10), ' ', '23:59:59'))
                    or (substring(accept_time, 12, 2) >= 16 and
                        ...
                        and sheet_no is not null), 1, 0) as fenzi,
       case when sheet_no is not null  then 1  else 0 end as fenmu
from dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp_js d_t left join dc_dwd.zhijai_temp_js10 t_a on t_a.xz_order=d_t.sheet_no;
