...
drop table dc_dwd.zhijia_person_clcs_base_detail_1101_10_temp;
create table dc_dwd.zhijia_person_clcs_base_detail_1101_10_temp as
select a.sheet_code,
       a.two_answer,
       accept_time,
       create_time,
...
from (select *, row_number() over (partition by id order by dts_kaf_offset desc) rn
      ...
      where province_name is not null
        and month_id in ('202310')
        and one_answer not like '%4%'
        and one_answer != '') a
where a.rn = '1'
  and 1 > 2;

insert overwrite table dc_dwd.zhijia_person_clcs_base_detail_1101_10_temp
select a.sheet_code,
       a.two_answer,
       accept_time,
       create_time,
...
where a.rn = '1';
select *
from dc_dwd.zhijia_person_clcs_base_detail_1101_10_temp;

-- todo 创建用于计算的存量测速满意率个人表
-- 建表
drop table dc_dwd.dwd_d_evt_zj_clcs_person_calculate_detail_20231101_10;
create table dc_dwd.dwd_d_evt_zj_clcs_person_calculate_detail_20231101_10 as
select *,
...
from (select t_a.prov_name,
      ...
      from dc_dwd.zhijia_info_temp10 t_a
               left join dc_dwd.zhijia_person_clcs_base_detail_1101_10_temp d_t on t_a.device_number = d_t.phone
      union all
      select t_a.prov_name,
             ...
             d_t.create_time
      from dc_dwd.zhijia_info_temp10 t_a
               left join dc_dwd.zhijia_person_clcs_base_detail_1101_10_temp d_t
      where locate(t_a.xz_order, d_t.sheet_code) > 0
        and t_a.xz_order is not null
        and t_a.xz_order != '') A
where 1 > 2;
-- 插入数据
insert overwrite table dc_dwd.dwd_d_evt_zj_clcs_person_calculate_detail_20231101_10
select *,
       row_number() over (partition by staff_code,device_number,xz_order,phone,sheet_code order by create_time desc) S_RN
from (select t_a.prov_name,
             t_a.city,
...
select *
from dc_dwd.dwd_d_evt_zj_clcs_person_calculate_detail_20231101_10 where two_answer is not null;
-- todo 个人结果
select prov_name,
       city,
       county,
      ...
      from (select prov_name, city, county, banzu, staff_code, xm
            from dc_dwd.zhijia_info_temp10) o_a
               left join (select case when two_answer is not null and two_answer rlike '1' then 1 else 0 end as fenzi,
                                 ...
                                 d.xm,
                                 d.staff_code
                          from dc_dwd.dwd_d_evt_zj_clcs_person_calculate_detail_20231101_10 d
