...
select th.trade_id,
     ...
     from dc_dwd.dwd_d_evt_cb_trade_his_10 th left join dc_dwd.zhijia_info_temp10 zp on th.trade_id = zp.zj_order ) a group by prov_name, city, county, banzu, staff_code,xm;



drop table dc_dwd.dwd_d_evt_zj_drt_person_detail_112_10;
create table dc_dwd.dwd_d_evt_zj_drt_person_detail_112_10 as
...
dc_dwd.zhijia_info_temp10 zp
left join dc_dwd.dwd_d_evt_cb_trade_his_10 th on zp.device_number=th.device_number
union all
select zp.prov_name,zp.city,zp.county,zp.banzu,zp.staff_code,zp.xm,zp.device_number,zp.zj_order,th.trade_id,th.accept_date,th.finish_date from
dc_dwd.zhijia_info_temp10 zp
left join dc_dwd.dwd_d_evt_cb_trade_his_10 th on zp.device_number=th.device_number where locate(zp.zj_order,th.trade_id)>0  and zp.zj_order is not null and zp.zj_order!=''
) A where 1>2;

insert overwrite table dc_dwd.dwd_d_evt_zj_drt_person_detail_112_10
select *,ROW_NUMBER() OVER(PARTITION BY staff_code,zj_order,trade_id ORDER BY accept_date desc) S_RN
from (
select zp.prov_name,zp.city,zp.county,zp.banzu,zp.staff_code,zp.xm,zp.device_number,zp.zj_order,th.trade_id,th.accept_date,th.finish_date from
dc_dwd.zhijia_info_temp10 zp
...
select zp.prov_name,zp.city,zp.county,zp.banzu,zp.staff_code,zp.xm,zp.device_number,zp.zj_order,th.trade_id,th.accept_date,th.finish_date from
dc_dwd.zhijia_info_temp10 zp
left join dc_dwd.dwd_d_evt_cb_trade_his_10 th on  zp.zj_order = th.trade_id  and zp.zj_order is not null and zp.zj_order!=''
) A;




select count(*) from dc_dwd.dwd_d_evt_zj_drt_person_detail_112_10;
...
select if((from_unixtime(unix_timestamp(accept_date, 'yyyyMMddHHmmss'), 'H') < 16 and
       ...
       case when trade_id is not null  then 1  else 0 end as zhuangji_total,
        ...
        d.xm,
        d.staff_code
        from dc_dwd.dwd_d_evt_zj_drt_person_detail_112_10 d where S_RN=1
...
select  prov_name,
      ...
      from (
               select if((from_unixtime(unix_timestamp(accept_date, 'yyyyMMddHHmmss'), 'H') < 16 and
                                 ...
                                 d.city,
                                 d.staff_code
                          from dc_dwd.dwd_d_evt_zj_drt_person_detail_112_10 d
                          where S_RN = 1 ) o_b
