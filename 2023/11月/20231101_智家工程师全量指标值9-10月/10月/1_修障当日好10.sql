...
select * from dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp;
...
from (
   ...
   left join dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp d_t on t_a.device_number=d_t.busi_no
   union all
   select
     t_a.prov_name,
     t_a.city,
     ...
     d_t.sheet_no,d_t.busi_no,d_t.accept_time,d_t.archived_time
   from dc_dwd.zhijia_info_temp10 t_a
   left join dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp d_t  where locate(t_a.xz_order,d_t.sheet_no)>0 and t_a.xz_order is not null and t_a.xz_order!=''
) A where 1>2;
-- todo 插入数据
...
from (
   ...
   select
     ...
     d_t.sheet_no,d_t.busi_no,d_t.accept_time,d_t.archived_time
   from dc_dwd.zhijia_info_temp10 t_a
   left join dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp d_t  on t_a.xz_order=d_t.sheet_no and t_a.xz_order is not null and t_a.xz_order!=''
) A;
    ...
    if((substring(accept_time, 12, 2) < 16 and
                 archived_time < concat(substring(date_add(accept_time, 0), 1, 10), ' ', '23:59:59'))
                    or (substring(accept_time, 12, 2) >= 16 and
                        ...
                        and sheet_no is not null), 1, 0) as fenzi,
       case when sheet_no is not null  then 1  else 0 end as fenmu
from dc_dwd.dwd_d_evt_zj_xz_detail_20231101_10_temp d_t left join dc_dwd.zhijia_info_temp10 t_a on t_a.xz_order=d_t.sheet_no)a group by prov_name, city, county, banzu, xm, staff_code;
