...
from (select *
     ...
     (select user_code,
      ...
      where (dt_id like '202307%' or dt_id like '202308%' or dt_id like '202309%' or dt_id like '202310%')
      ...
      from (select user_code,
           ...
           (select accept_user,
            ...
            from (select compl_prov,
                  ...
                  where (month_id in ('202307', '202308', '202310') and day_id = '31')
                                 or (month_id in ('202309') and day_id = '30')
      ...
      from (select sheet_id,
           ...
           (select sheet_no,
            ...
            where (
                ...
                ( month_id = '202310' and day_id = '01' and acc_month = '202309' )
                )
...
select a.prov_name,
       ...
       (fenzi / fenmu),
       '202307080910'
from (select * from dc_dwd.team_1101 where window_type = '热线客服代表/投诉处理' and prov_name !='客户服务部') a
         left join
      ...
      from (select sheet_id, sheet_no, proce_user_code
        ...
        and cust_level_name not rlike '五星')
                           ...
                           where accept_time is not null
                             ...
                             and dt_id in ('20230731', '20230831', '20230930', '20231031')
                             and (archived_time like '%2023-07%' or archived_time like '%2023-08%' or archived_time like '%2023-09%' or archived_time like '%2023-10%')) b
