...
select cust_area_name,
       '修障履约及时率' as kpi_name,
       sum(case
               when round((unix_timestamp(arrive_time, 'yyyyMMddHHmmss') - unix_timestamp(book_time)) / 60, 2) <
                    30 then 1
               else 0 end)                                                                                   as fenzi,
...
from (select sheet_no, sheet_id, accept_time, cust_area, cust_area_name
      ...
      where a.month_id in ('202307')
        ...
        and a.serv_type_name in ('故障工单（2021版）>>融合>>【网络使用】宽带网络>>宽带无法上网',
         ...
         left join(select kf_sn, arrive_time,book_time, prov_id, province
...
------------明细
select   sheet_no, accept_time,deal_man, cust_area, cust_area_name,kf_sn, arrive_time,book_time, prov_id, province
from (select sheet_no, sheet_id, accept_time, cust_area, cust_area_name
      from dc_dwa.dwa_d_sheet_main_history_chinese a
      where a.month_id in ('202307')
        and a.cust_area_name = '重庆'
        and a.is_delete = '0'
        and a.sheet_type = '04'
        and (pro_id not in ("S1", "S2", "N1", "N2")
          or (pro_id in ("S1", "S2", "N1", "N2")
        ...
        and a.serv_type_name in ('故障工单（2021版）>>融合>>【网络使用】宽带网络>>宽带无法上网',
                                 ...
                                 '故障工单（2021版）>>融合>>【网络使用】宽带网络>>突发故障--宽带网络故障',
                                 '故障工单（2021版）>>宽带>>【网络使用】宽带网络>>突发故障--宽带网络故障')) t1
         left join(select kf_sn, arrive_time,book_time, prov_id, province,deal_man
                   from dc_dwd.dwd_m_evt_oss_insta_secur_gd
                   where month_id = '202307') t2
                  on t1.sheet_no = t2.kf_sn
;
