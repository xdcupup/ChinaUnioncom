...
create table dc_dwd.lz_gold_silver_20231103 as
select sheet_id,
       sheet_no,
       sheet_type,
...
where month_id in ('202310')
  and (
    regexp(upper(serv_content),'乐铃|彩铃|铃声|铃音|炫铃|助理|来电名片|漏话提醒|挂机短信|语音留言|智能应答|反诈名片|数字名片|云盘|联通看家|神眼|沃家云视|组网|FTTR|全屋光宽带|全屋光纤|全屋WIFI|智慧到家|联通超清|IPTV|视频名片|5G消息|AI秘书|XR通话|数字人|畅视多视频|.*哔哩.*定向流量包.*|.*芒果.*定向流量包.*|.*腾讯.*定向流量包.*|.*优酷.*定向流量包.*|.*爱奇艺.*定向流量包.*|.*阿里.*定向流量包.*|.*百度.*定向流量包.*|.*抖音.*定向流量包.*|.*定向流量.*5G加油包.*|沃视频|5G宽视界|手机视频|家庭会员|联通学堂|.*沃音乐.*精英.*|.*沃音乐.*家庭.*|.*沃音乐.*成长.*|联通畅游|沃畅游|联通爱听|加速宝|视频客服|视频会议|AI翻译|5G消息|互联网电视')= true
    or
...
select * from dc_dim.dim_four_a_little_product_code;
--------------------------------
-- ------投诉
set hive.mapred.mode = nonstrict;
set mapreduce.job.queuename = q_dc_dw;
select sheet_pro_name,
       ...
       month_id,
       '投诉'
from dc_dwd.lz_gold_silver_20231103 tb1
