...
with t1 as (select sheet_no,
 ...
 when regexp(serv_content,'沃音乐')=true and regexp(serv_content,'铂金')=true then '视频彩铃'
  ...
  when regexp(upper(serv_content),'组网|FTTR|全屋光宽带|全屋光纤|全屋WIFI|智慧到家')=true then '联通组网' end as cp_type,
  case
           when sheet_type = '01' and is_shensu != '1'  then '投诉'
           when is_shensu = '1' and accept_channel_name = '工信部申诉-预处理' then '申诉' else'其他'
           end                                             as com_appeal_classification,
       case
           when accept_channel = '13' and submit_channel = '05' and pro_id = 'AA'  and
                serv_type_name is not null then '舆情'else'其他' end as public_sentiment_classification,
                   case
            ...
            where
                ...
                , '乐铃|彩铃|铃声|铃音|炫铃|云盘|组网|FTTR|全屋光宽带|全屋光纤|全屋WIFI|智慧到家')= true
               or
                (regexp (serv_content
                , '沃音乐')= true
     ...
     t2 as (select sheet_no,
                   ...
                   cp_type,
                   month_id,
                   com_appeal_classification,
                   public_sentiment_classification
            from t1
...
from t2
;
drop table dc_Dwd.test12345;
create table dc_Dwd.test12345 as
with t1 as (select sheet_id,
                   ...
                   contact_id,                                   -- 接触记录id
                   cust_city,                                    -- 号码归属地市编码
                   cust_city_name,                               -- 号码归属地市名称
                   busno_prov_id,                                -- 业务号码归属省分编码
                   busno_prov_name,                              -- 业务号码归属省分名称
                   caller_no,                                    -- 主叫号码
                   busi_no,                                      -- 业务号码
                   contact_no,                                   --联系电话
                   contact_no2,                                  --联系电话2
 ...
 when regexp(serv_content,'沃音乐')=true and regexp(serv_content,'铂金')=true then '视频彩铃'
  when regexp(upper(serv_content),'云盘')=true then '联通云盘'
  when regexp(upper(serv_content),'组网|FTTR|全屋光宽带|全屋光纤|全屋WIFI|智慧到家')=true then '联通组网' end  as product_type,
...
case
            ...
            -- or (sheet_type='09' and regexp(serv_type_name,'申诉工单>>预处理') and appeal_status not in ('01','02','06','07') )  -- 20230208 注释掉申诉工单
              ...
              and nvl(serv_content
                , '') not like '%软研院自动化测试%' -- 剔除掉测试数据
)
select *
from t1;

select distinct product_type
from dc_Dwd.test12345;
---- 投诉
select sheet_no,
       serv_type_name,
       serv_content,
