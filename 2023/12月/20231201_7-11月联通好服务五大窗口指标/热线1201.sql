     ...
     (select a.proce_user_code,
      ...
      from (select sheet_id,
           ...
           (select sheet_no,
                   ...
                   cp_timely_contact
            from dc_dm.dm_d_de_gpzfw_yxcs_acc
            where ((month_id = '202308' and day_id = '03' and acc_month = '202307') or
                   (month_id = '202309' and day_id = '03' and acc_month = '202308') or
                   (month_id = '202310' and day_id = '01' and acc_month = '202309') or
                   (month_id = '202311' and day_id = '01' and acc_month = '202310'))
              and nvl(serv_content, '') not like '%软研院自动化测试%'
              and sheet_type_name = '投诉工单'
...
select a.prov_name,
       ...
       (fenzi / fenmu),
       '20230708091011'
from (select * from dc_dwd.team_1201 where window_type = '热线客服代表/投诉处理' and prov_name != '客户服务部') a
         left join
