with t1 as (select 区域,
                   ...
                   未纠正客户错误信息,
                   问题小计,
                   (抽检总量 - 问题小计) / 抽检总量 as `听得懂合规率（目标值≥95%）`
            from (select 区域,
                         count(*)                                                                            as `抽检总量`,
                  ...
                  from fwbz.day100_0425) t1
            order by case
                         when 区域 = '全国' then 1
                         when 区域 = '北方一中心' then 2
                         when 区域 = '北方二中心' then 3
                         when 区域 = '南方一中心' then 4
                         when 区域 = '南方二中心' then 5 end),
     t2 as (select 区域,
                   抽检总量,
                   超长等候未告知,
                   `沟通/受理能力差`,
                   ...
                   介绍错误或关键信息不全,
                   问题小计,
                   (抽检总量 - 问题小计) / 抽检总量 as `讲得清合规率（目标值≥95%）`
            from (select 区域,
                         count(*)                                                                                as `抽检总量`,
     ...
     t3 as (select 区域,
                   ...
                   工单记录信息有误,
                   问题小计,
                   (抽检总量 - 问题小计) / 抽检总量 as `解得对合规率（目标值≥95%）`
            from (select 区域,
                         count(*)                                                                                          as `抽检总量`,
...
select t1.区域,
       ...
       `解得对合规率（目标值≥95%）`
from t1
         join t2 on t1.区域 = t2.区域
         join t3 on t1.区域 = t3.区域
 order by case
                         when t1.区域 = '全国' then 1
                         when t1.区域 = '北方一中心' then 2
                         when t1.区域 = '北方二中心' then 3
                         when t1.区域 = '南方一中心' then 4
                         when t1.区域 = '南方二中心' then 5 end
;