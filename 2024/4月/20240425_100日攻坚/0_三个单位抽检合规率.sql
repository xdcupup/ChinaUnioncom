...
from t1
         join t2 on t1.抽检单位 = t2.抽检单位
         join t3 on t1.抽检单位 = t3.抽检单位
order by case
             when t1.抽检单位 = '热线' then 1
             when t1.抽检单位 = '升投' then 2
             when t1.抽检单位 = '客户体验' then 3
             when t1.抽检单位 = '合计' then 4
             end
;
