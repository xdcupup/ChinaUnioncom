...
from (select user_number, scene_id, date_id
      ...
      where scene_id in (
                         '29210451803808',  -- 仓山市民服务中心
                         '29210388017312',  -- 中海环宇城购物中心
                         '29210431748768',  -- 麦德龙(淄博张店商场)
                         '29035465449120'  -- 雅侎之星购物中心
          )
        and date_id >= '20240401') a
         left join
     (select a.*, notifyid
      from (select * from dc_src_rt.rpt_ivr_click_record where date_id >= '202404012') a
               join
           (select distinct id,
                            notifyid,
                            date_id
            from dc_src_rt.cti_cdr_bak
            where date_id >= '20240412'
              and notifyid in (
...
drop table dc_Dwd.ywcp_mx_temp_4sc;
--明细表
create table dc_Dwd.ywcp_mx_temp_4sc as
select sf_name,
       ds_name,
...
from (select sf_name,
      ...
      from dc_dwd.ywcp_temp_3 aa
               left join (select serial_number,
                          ...
                          where a.id in (
                              '35000093486839','37000015739074','37000098367255','5300008654536'
                              )) bb
                         on aa.user_number = bb.serial_number
               left join dc_dwd.scence_prov_code cc
                         on bb.province_code = cc.sf_code and bb.eparchy_code = cc.ds_code
      ) dd;


select *
from dc_Dwd.ywcp_mx_temp_4sc where scenename = '福州_仓山市民服务中心';
