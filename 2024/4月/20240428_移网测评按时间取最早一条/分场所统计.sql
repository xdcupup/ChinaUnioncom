...
with t1 as (select sf_name,
            ...
            from (select distinct *
                  from (select *
                        from dc_dwd.ywcp_mx_temp_final) cc) aa
...
with t1 as (select if(scen_type in ('商务楼宇', '酒店'), '商务楼宇及酒店', scen_type) as scen_type,
                   count(*)                                                           as `发送量`,
                   count(if(aj_1 in ('1', '2', '3'), 1, null))                        as `参评量`,
                   round(count(if(aj_1 in ('1', '2', '3'), 1, null)) / count(*), 6)   as `参评率`,
                   count(if(yymy = '满意', 1, null))                                  as `语音满意量`,
            ...
            from (select distinct *
                  from (select *
                        from dc_dwd.ywcp_mx_temp_final) cc) aa
            group by if(scen_type in ('商务楼宇', '酒店'), '商务楼宇及酒店', scen_type)),
     t2 as (select scen_type,
                   `发送量`,
                   `参评量`,
                   `参评率`,
                   `语音满意量`,
                   ...
                   `上网卡顿、不稳定量`,
                   `上网其他量`
            from t1  where scen_type is not null)
select *
from t2;