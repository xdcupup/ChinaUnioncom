...
from (select *
      ...
      where window_type = '热线客服代表/投诉处理') t1
         left join(select id,
                          cp_month,
                          window_type,
                          prov_name,
                   ...
                   from (select id,
                         ...
                         group by id, cp_month, window_type, prov_name, city_name, stuff_id, stuff_name,
                                  month_id) t2) t2 on t1.stuff_id = t2.stuff_id
         left join (select id,
                           cp_month,
                           window_type,
                           prov_name,
                                ...
                                from (select proce_user_code,
                                     ...
                                     (select *
                                      ...
                                      where window_type = '热线客服代表/投诉处理'
                                        and prov_name != '客户服务部') bb on aa.proce_user_code = bb.stuff_id) t1
                          group by id, cp_month, window_type, prov_name, city_name, stuff_id, stuff_name,
                                   month_id) t2) t3 on t1.stuff_id = t3.stuff_id
         left join (select id,
                           cp_month,
                           window_type,
                           prov_name,
                                ...
                                from (select proce_user_code,
                                     ...
                                     (select *
                                      ...
                                      where window_type = '热线客服代表/投诉处理'
                                        and prov_name != '客户服务部') bb on aa.proce_user_code = bb.stuff_id) t1
                          group by id, cp_month, window_type, prov_name, city_name, stuff_id, stuff_name,
                                   month_id) t2) t4 on t1.stuff_id = t4.stuff_id
         left join (select id,
                           cp_month,
                           window_type,
                           prov_name,
                    ...
                    group by id, cp_month, window_type, prov_name, city_name, stuff_id, stuff_name, month_id) t5
