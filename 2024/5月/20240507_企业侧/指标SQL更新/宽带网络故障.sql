...
from (select prov_name,
      ...
      group by prov_name,
         ...
         left join (select code,
                           ...
                           region_code,
                           region_name
                    from dc_dim.dim_province_code) b on a.prov_name = b.meaning
group by b.code
union all
select '00'              prov_code,
       '00'              code_value,
...
from (select meaning                                                   as pro_name,            --省份名称
             ...
             '网络可用故障少*'                                             as index_level_4,       -- 指标级别四
             'FWBZ242'                                                 as kpi_code,            --指标编码
             '宽带网络故障'                                            as index_name,          --五级-指标项名称
             ...
             round(index_value_numerator / index_value_denominator, 6) as index_value
      from (select meaning,
                   code_value,
                   userScore,
                   index_value_denominator,
                   index_value_numerator
            from (select b.code            prov_code,
                         '00'              code_value,
                         round(
                                 sum(total_source) / (
                                     sum(bad_one_scene) + sum(center_one_scene) + sum(good_one_scene)
