...
with t1 as (select sf_name,
                   ...
                   count(*)                                                           as `发送量`,
                   count(if( Q1 in ('1', '2', '3'), 1, null))                          as `参评量`,
                   round(count(if( Q1 in ('1', '2', '3'), 1, null)) / count(*), 6)   as `参评率`,
            ...
            from (select distinct *
                  from (select *
                        from dc_Dwd.ywcp_mx_temp_final_b) cc) aa
     ...
     t2 as (select sf_name,
                   ...
                   `上网卡顿、不稳定提及率`,
                   `上网其他提及率`,
                   if(`语音满意率` >= 0.7, '是', '否')     as `是否语音满意场所`,
                   if(`手机上网满意率` >= 0.7, '是', '否') as `是否手机上网满意场所`,
                   `电话无法打通、接通量`,
                   `通话有杂音、听不清量`,
                   `通话经常中断量`,
                   `语音其他量`,
