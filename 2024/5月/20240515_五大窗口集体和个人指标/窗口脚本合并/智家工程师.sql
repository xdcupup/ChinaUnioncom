               ...
               where index_name in ('装机当日通率')
                 and month_id = '${v_month_id}') a
                  right join (select *
                              ...
                              where window_type = '智家工程师') b on a.stuff_id = b.stuff_id
         group by b.cp_month, b.window_type, b.prov_name, b.city_name, b.stuff_id, b.stuff_name
         union all
-- 修障当日好
         select b.cp_month,
                b.window_type,
...
-- 宽带测速满意率
         ...
         from (select cp_month,
               ...
               from (select sheet_no,
                     ...
                     where bus_sort = '宽带修障'
                       and (date_id rlike '202404' or date_id rlike '202403')) a
                        right join (select *
                                    from dc_dwd.zhijia_good_serivce_person_${v_month_id}) b
                                   on a.sheet_no = b.kf_sn
               group by cp_month, window_type, prov_name, city_name, stuff_id, stuff_name) aa
         group by cp_month, window_type, prov_name, city_name, stuff_id, stuff_name
         union all
-- 宽带测速感知率
         select cp_month,
         ...
         from (select cp_month,
               ...
               from (select sheet_no,
                     ...
                     from dc_dm.dm_d_cem_broadband_cl_speed_details_report
                     where bus_sort = '宽带修障'
                       and (date_id rlike '202403' or date_id rlike '202403')) a
                        right join (select *
                                    from dc_dwd.zhijia_good_serivce_person_${v_month_id}) b
                                   on a.sheet_no = b.kf_sn
               group by cp_month, window_type, prov_name, city_name, stuff_id, stuff_name) tb1
         group by cp_month, window_type, prov_name, city_name, stuff_id, stuff_name
         union all
-- 装机预约响应满意率
         select cp_month,
         ...
         from (select trade_id,
               ...
               where (dt_id rlike '202403' or dt_id rlike '202404')) a
                  right join (select *
...
-- 修障服务响应率
         ...
         from (select sheet_code,
               ...
               where month_id in ('202403', '202404')) a
                  right join (select *
                              from dc_dwd.zhijia_good_serivce_person_${v_month_id}
                              where zw_type = '修障装维') b
                             on a.sheet_code = b.kf_sn
         group by cp_month, window_type, prov_name, city_name, stuff_id, stuff_name) t1;
...
from (select b.cp_month,
      ...
      from (select cp_month,
            ...
            from (select sheet_no,
                  ...
                  from dc_dm.dm_d_cem_broadband_cl_speed_details_report
                  where bus_sort = '宽带修障'
                    and (date_id rlike '202403' or date_id rlike '202404')) a
                     right join (select *
