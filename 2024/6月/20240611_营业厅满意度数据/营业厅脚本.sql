...
select trade_id from dc_dwd.yyt_detailed_statement where chnl_tp_lv1_name = '自有' and chnl_tp_lv2_name = '实体';

select * from dc_dwd.yyt_detailed_statement where date_id rlike '202405';
select
	a.province_name,
	'全部' as eparchy_name,
	max(fsl) fsl, --发送量
	...
	sum(case when a.yythj is not null or a.pddhsc is not null or  a.czlc_blsc is not null or a.yyyfwtdjn is not null or a.yyyywtjsyx is not null or a.qt is not null then 1 end) yythjtjlfm,--营业厅环境、设备价提及率分母
	------------------------------------------------------------参与用户------------------------------------------------------------
	...
	count(case when a.yyyywtjsyx is not null and a.answer_1 in ('9','10') then 1 end) yyyywtjsyxztj, --营业员业务推荐适宜性推荐用户
	...
	------------------------------------------------------------贬损用户------------------------------------------------------------
	count(case when a.yythj is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) yythjbs, --营业厅环境、设备价贬损用户
	count(case when a.pddhsc is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) pddhscbs, --排队等候时长贬损用户
	count(case when a.czlc_blsc is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) czlc_blscbs, --操作流程、办理时长贬损用户
	count(case when a.yyyfwtdjn is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) yyyfwtdjnbs, --营业员服务态度技能贬损用户
	count(case when a.yyyywtjsyx is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) yyyywtjsyxbs, --营业员业务推荐适宜性贬损用户
	...
	count(case when a.yyyywtjsyx is not null and a.answer_1 in ('9','10') then 1 end) yyyywtjsyxmylfz, --营业员业务推荐适宜性满意率分子
	...
	------------------------------------------------------------不满意------------------------------------------------------------
	count(case when a.yythj is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) yythjbmylfz, --营业厅环境、设备价不满意率分子
	count(case when a.yythj is not null then 1 end) yythjbmylfm,--营业厅环境、设备价不满意率分母
	count(case when a.pddhsc is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) pddhscbmylfz, --排队等候时长不满意率分子
	count(case when a.pddhsc is not null then 1 end) pddhscbmylfm,--排队等候时长不满意率分母
	count(case when a.czlc_blsc is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) czlc_blscbmylfz, --操作流程、办理时长不满意率分子
	count(case when a.czlc_blsc is not null then 1 end) czlc_blscbmylfm,--操作流程、办理时长不满意率分母
	count(case when a.yyyfwtdjn is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) yyyfwtdjnbmylfz, --营业员服务态度技能不满意率分子
	count(case when a.yyyfwtdjn is not null then 1 end) yyyfwtdjnbmylfm,--营业员服务态度技能不满意率分母
	count(case when a.yyyywtjsyx is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) yyyywtjsyxbmylfz, --营业员业务推荐适宜性不满意率分子
	count(case when a.yyyywtjsyx is not null then 1 end) yyyywtjsyxbmylfm,--营业员业务推荐适宜性不满意率分母
	count(case when a.qt is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) qtbmylfz, --其他不满意率分子
	count(case when a.qt is not null then 1 end) qtbmylfm, --其他不满意率分母
	'' as by_1,
	'' as by_2,
	'' as by_3
from
	dc_dwd.yyt_detailed_statement  a
left join
	(
		SELECT
			cb1.prov_name,
		...
		FROM
			...
			where date_id rlike '${v_month_id}' and scene_id='28502833752736'
			) c
		left join
				(
				select
...
select
	...
	------------------------------------------------------------贬损用户------------------------------------------------------------
	count(case when a.yythj is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) yythjbs, --营业厅环境、设备价贬损用户
	count(case when a.pddhsc is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) pddhscbs, --���话信号稳定性贬损用户
	count(case when a.czlc_blsc is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) czlc_blscbs, --操作流程、办理时长贬损用户
	count(case when a.yyyfwtdjn is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) yyyfwtdjnbs, --营业员服务态度技能贬损用户
	count(case when a.yyyywtjsyx is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) yyyywtjsyxbs, --营业员业务推荐适宜性贬损用户
	count(case when a.qt is not null and a.answer_1 in ('1','2','3','4','5','6') then 1 end) qtbs,--其他贬损用户
...
from
	dc_dwd.yyt_detailed_statement a
where  date_id rlike '${v_month_id}'   ) a
left join
		(
		select
			'全国' as prov_name,
			'全部' as area_name,
			COUNT(c.question_id) fsl
		from dc_dwd.evaluation_send_sms_log  c
		where  date_id rlike '${v_month_id}'  and scene_id='28502833752736'
		)  b
