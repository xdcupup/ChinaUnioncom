...
-- hdfs dfs -put /home/dc_dw/new_product_list.csv  /user/dc_dw
...
drop table if exists dc_dwd.xdc_184_serv_type_name;
create table if not exists dc_dwd.xdc_184_serv_type_name
(
    serv_type_name string
) row format delimited fields terminated by ',' stored as textfile
    location 'hdfs://beh/user/dc_dwd/dc_dwd.db/xdc_184_serv_type_name';
-- hdfs dfs -put /home/dc_dw/xdc_data/xdc_184_serv_type_name.csv /user/dc_dw

load data inpath '/user/dc_dw/xdc_184_serv_type_name.csv' overwrite into table dc_dwd.xdc_184_serv_type_name;

select *
from dc_dwd.xdc_184_serv_type_name;
...
with t1 as (select sheet_id,
     ...
     t2 as (select sheet_id,
                   ...
                   proc_name
            from t1
                     join dc_dwd.xdc_184_serv_type_name xdc on xdc.serv_type_name = t1.serv_type_name),
...
with t1 as (select sheet_id,
     ...
     t3 as (select sheet_id,
            ...
            from t2
                     join dc_dwd.new_product_list aa on aa.proc_name = t2.proc_name)
select sheet_no, serv_type_name, proc_name, serv_content ,'投诉' as type_1
from t3
where sheet_type = '01'
  and is_shensu != '1'
;
...
with t1 as (select sheet_id,
     ...
     t3 as (select sheet_id,
            ...
            from t2
                     join dc_dwd.new_product_list aa on aa.proc_name = t2.proc_name)
select sheet_no, serv_type_name, proc_name, serv_content ,'申诉' as type_1
from t3
where  is_shensu = '1'
  and accept_channel_name = '工信部申诉-预处理'
;
...
with t1 as (select sheet_id,
     ...
     t3 as (select sheet_id,
            ...
            from t2
                     join dc_dwd.new_product_list aa on aa.proc_name = t2.proc_name)
select sheet_no, serv_type_name, proc_name, serv_content ,'舆情' as type_1
from t3
where accept_channel = '13'
  and submit_channel = '05'
  and pro_id = 'AA'
