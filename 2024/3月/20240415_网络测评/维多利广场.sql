...
from (select user_number, scene_id, date_id
      ...
      where scene_id in (
        ...
        and date_id = '20240415') a
         left join
     (select a.*, notifyid
      from (select * from dc_src_rt.rpt_ivr_click_record where date_id = '20240415') a
               join
           (select distinct id,
                            notifyid,
                            date_id
            from dc_src_rt.cti_cdr
            where date_id = '20240415'
              and notifyid in (
...
drop table dc_Dwd.ywcp_mx_temp_vdl;
--明细表
create table dc_Dwd.ywcp_mx_temp_vdl as
select sf_name,
       ds_name,
