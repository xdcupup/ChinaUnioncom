...
insert overwrite table dc_dm.dm_service_standard_index partition (monthid = '${v_month_id}', index_code = 'zqkhjltsl')
select index_level_1,
       ...
       index_value
from dc_dm.dm_service_standard_index
where index_name = '政企客户经理投诉率'
  and monthid = '202401'
  and index_code = 'zqkhjltsl';
...
with t1 as (select index_name, b.meaning as pro_name
     ...
     t2 as (select *
            ...
            where monthid = '202402'
              and month_id != '202401'
              and index_name = '政企客户经理投诉率'),                                              --自己出的
     t3 as (select *
            from dc_dwd.dwd_standard_res
            where monthid = '202402'
              and index_name = '政企客户经理投诉率'),                                              -- 报表的
       ...
       case
                                           ...
                                           when rule in ('=', '＝')
                                               ...
                                               case
                                                   when cast(index_value as decimal(32, 2)) =
                                                        cast(target_value_nation as decimal(32, 2)) then '是'
           ...
           when pro_name != '全国' then case
                                            when rule in ('=', '＝')
                                                ...
                                                case
                                                    when cast(index_value as decimal(32, 2)) =
                                                         cast(target_value_prov as decimal(32, 2)) then '是'
                                                    when index_value = '--' then '--'
                                                    else '否' end
                                            when rule = '≥'
                                                then case
...
from (select index_level_1,
      ...
      where (monthid = '202402' and index_name != '政企客户经理投诉率')
--           and pro_name = '全国'
          and index_name != '移网网络卡顿投诉率'
         or (index_name = '政企客户经理投诉率' and month_id = '202401自己出'
--           and pro_name = '全国'
          )) a
