...
select tb1.meaning,
       tb1.vip_class_id,
       tb2.cnt          as cnt_1, --`成功量（自助申请）`,
       tb2.user_cnt     as cnt_2, --`成功用户量`,
       tb2.jjkj_my_rate as cnt_3, --`自助侧紧急开机满意率`,
       tb3.cnt          as cnt_4, --`满足紧急开机话务量（自助申请紧急开机话务）`,
       tb3.user_cnt     as cnt_5, --`申请用户量（符合紧急开机条件）`,
       tb4.cnt          as cnt_6, --`话务量（自助申请紧急开机话务）`,
       tb4.user_num     as cnt_7 --`申请用户量`
from dc_dwd.start_xdc_temp tb1
         left join (select
                           nvl(meaning,'--') as meaning,
...
--6-7
                          ...
                          from (select meaning,
                                ...
                                from (select meaning,
                                      ...
                                      group by t1.meaning, vip_class_id) b
                                where vip_class_id is not null) t1
                          where vip_class_id in ('600', '700')) bb) tb2
                   on tb1.meaning = tb2.meaning and tb1.vip_class_id = tb2.vip_class_id
         left join(select
                          nvl(meaning,'--') as meaning,
                          nvl(vip_class_id,'--') as vip_class_id,
                          nvl(cnt,'--') as cnt,
                          nvl(user_cnt,'--') as user_cnt from (
--申请用户量（符合紧急开机条件）
