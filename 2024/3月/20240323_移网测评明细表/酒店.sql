...
--   and notifyid = 'f8d13a2c08a54d8bb3c5f0e00141fdb6';
--外呼任务

-- 住宅小区		group_source_1711107371504
-- 商务楼宇		group_source_1711107372208
-- 酒店	group_source_1711107372774
-- 酒店


drop table dc_dwd.jd_temp_01;
create table dc_dwd.jd_temp_01 as
select *, row_number() over (partition by user_number order by ordernum) as rn1
from (select user_number from dc_src_rt.eval_send_sms_log where date_id = '20240323' and scene_id = '14268300000002') a
         left join
     (select a.*
      from (select * from dc_src_rt.rpt_ivr_click_record where date_id = '20240323') a
           ...
           (select distinct id
            from dc_src_rt.cti_cdr
            where date_id = '20240323'
              and notifyid = 'f8d13a2c08a54d8bb3c5f0e00141fdb6') b
           on a.callid = b.id) b
     on a.user_number = b.callnumber;



drop table dc_dwd.jd_xdc_tab1;
create table dc_dwd.jd_xdc_tab1 as
select user_number, collect_list(keyno) as source
from dc_dwd.jd_temp_01
group by user_number;

drop table dc_dwd.jd_xdc_tab2;
create table dc_dwd.jd_xdc_tab2 as
...
from dc_dwd.jd_xdc_tab1 a
         left join(select * from dc_dwd.jd_temp_01 where rn1 = 1) b on a.user_number = b.user_number;



drop table dc_Dwd.jd_mx_temp;
create table dc_Dwd.jd_mx_temp as
select sf_name,
       ds_name,
       '酒店'                                                                                                         as scen_type,
       id,
...
from dc_dwd.jd_xdc_tab2 aa
         left join dc_Dwd.group_source_1711107372774 bb on aa.user_number = bb.serial_number
...
select *
from dc_Dwd.group_source_1711107372774;
select *
from dc_Dwd.group_source_1711107372208;