...
select message_send_time,user_number
from dc_src_rt.eval_send_sms_log where date_id = '20240324' and scene_id = '28355620440224'
; -- 下发表
...
drop table dc_dwd.jmzz_temp_01;
create table dc_dwd.jmzz_temp_01 as
select *, row_number() over (partition by user_number order by ordernum) as rn1
from (select user_number from dc_src_rt.eval_send_sms_log where date_id = '20240323' and scene_id = '14268300000000') a
         left join
     (select a.*
      from (select * from dc_src_rt.rpt_ivr_click_record where date_id = '20240323') a
           ...
           (select distinct id
            ...
            where date_id = '20240323'
              and notifyid = 'e33afef6a500433b9ab6708500d37f4b') b
           on a.callid = b.id) b
     on a.user_number = b.callnumber;
select * from dc_dwd.jmzz_temp_01;


drop table dc_dwd.xdc_tab1;
create table dc_dwd.xdc_tab1 as
select user_number, collect_list(keyno) as source
from dc_dwd.jmzz_temp_01
group by user_number;

select * from dc_dwd.xdc_tab1;

drop table dc_dwd.xdc_tab2;
create table dc_dwd.xdc_tab2 as
...
from dc_dwd.xdc_tab1 a
         left join(select * from dc_dwd.jmzz_temp_01 where rn1 = 1) b on a.user_number = b.user_number;

select * from dc_dwd.xdc_tab2;


drop table dc_Dwd.jmzz_mx_temp;
create table dc_Dwd.jmzz_mx_temp as
select sf_name,
       ds_name,
       '居民住宅'                                                                                                     as scen_type,
       id,
