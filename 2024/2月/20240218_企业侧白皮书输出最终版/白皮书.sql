...
desc  dc_dwd.dwd_standard_res;
show partitions dc_dwd.dwd_standard_res;
-- todo 导入指标 1月份报表结果
-- hdfs dfs -put /home/dc_dw/xdc_data/dwd_standard_res_202402.csv /user/dc_dw
load data inpath '/user/dc_dw/dwd_standard_res_202402.csv' overwrite into table dc_dwd.dwd_standard_res partition (monthid = 202402);
select *
...
select a.pro_name,
       ...
       b.pro_name,
       b.index_name,
       b.index_value_numerator,
       b.index_value_denominator,
       b.index_value
from (select *
      from dc_dm.dm_service_standard_index
      where month_id = '202401') a
         left join dc_dwd.dwd_standard_res b
                   on a.index_name = b.index_name and a.pro_name = b.pro_name and a.monthid = b.monthid;
select *
from dc_dm.dm_service_standard_index
where index_name =
...
-- todo 插入 357 ivr 指标数据

-- hdfs dfs -put /home/dc_dw/xdc_data/357ivr.csv /user/dc_dw
load data inpath '/user/dc_dw/357ivr.csv' overwrite into table dc_dm.dm_service_standard_index partition (monthid = 202401,index_code = 'final_insert');
select *
from dc_dm.dm_service_standard_index
where monthid = 202401
  and index_code = 'final_insert';

with t1 as (select index_name, b.meaning as pro_name
            from dc_dwd.dwd_standard_index a
                     left join (select meaning
                                from dc_dim.dim_province_code
                                where region_code is not null
                                union all
                                select '全国' as meaning) b),
     t2 as (select * from dc_dm.dm_service_standard_index where month_id = '202401'),              --自己出的
     t3 as (select * from dc_dwd.dwd_standard_res where monthid = '202401'),                       -- 报表的
...
select index_level_1           as `指标级别一`,
       ...
       index_value             as `指标值`,
       monthid
from dc_dwd.dwd_standard_index_final where monthid = '202402';
