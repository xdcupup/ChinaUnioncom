...
-- hdfs dfs -put /home/dc_dw/xdc_data/product_list.csv /user/dc_dw

load data inpath '/user/dc_dw/product_list.csv' overwrite into table dc_dwd.new_product_list;
select *
...
where month_id in ('202401')
  and serv_type_name in ('投诉工单（2021版）>>移网>>【入网】咨询>>资费政策不清晰/不便于理解',
    ...
    t2 as ( select * from dc_dwd.new_product_list where proc_name in())
...
联通亲情宽带-300M（天津）
商务快线1500M[河北]
IPTV影视会员10元/月优惠专属含TV基础(长春)-立即生效
第二条宽带600M包月融合4  元（内蒙）2024
5G冰激凌移网60元（赤峰）
5G冰激凌移网129元（赤峰）
福墩墩（赤峰）
快递卡套餐59元档（陕西）
联通惠商光速商户版套餐成员商户专线1000M
