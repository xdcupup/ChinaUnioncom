...
#      *修改记录
#      *所属于实体
#      *来源表 dc_dm.dm_d_callin_sheet_contact_agent_report dc_dwa.dwa_d_sheet_main_history_chinese  dc_dim.dim_province_code
#      *目标表 dc_dwd.dwd_xdc_service_request_8scene_allmonth
#      ************************************************************************
#获取完整账期和单独的年,月,日
v_date=$1
#当月月份
v_month_id=`date -d  " " "+%Y%m"`
#上月月份
v_month_up=`date -d "last month" +%Y%m`
#本月第一天
v_last_dt_id=$(date -d "$(date +%Y-%m-01)" +"%d")
#本月最后一天
v_first_dt_id=$(date -d "$(date +'%Y-%m-01 +1 month -1 day')" +"%Y%m%d")
#当月第25天
v_25_dt_id=$(date +"%Y%m25")
hive -e"
set hive.mapred.mode = nonstrict;
set mapreduce.job.queuename = q_dc_dw;
insert  overwrite table dc_dwd.dwd_xdc_service_request_8scene_allmonth partition (monthid = '${v_month_id}')
-- todo 星级服务内容服务请求量-不满
