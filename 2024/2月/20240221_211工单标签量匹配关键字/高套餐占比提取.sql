...
-- 投诉工单（2021版）>>移网>>【产品及计费使用】计交费及发票>>套内外流量费用争议 7-1 投诉
-- 其中 价格 >= 99  分月

select count(*) as cnt, month_id
from dc_dwa.dwa_d_sheet_main_history_chinese
where month_id between '202307' and '202401'
  and serv_type_name = '投诉工单（2021版）>>移网>>【产品及计费使用】计交费及发票>>套内外流量费用争议'
  and proc_name like '%元%'
  and regexp_extract(proc_name, '(\\d+)元', 1) >= 99
group by month_id;
...
with t1 as (select *,
     ...
     t3 as (select *
            ...
            where t1.sheet_type = '01'
              ...
              and nvl(sheet_pro, '') != '')
select serv_type_name , month_id,count(*) as cnt
from t3
where serv_type_name = '投诉工单（2021版）>>移网>>【产品及计费使用】计交费及发票>>套内外流量费用争议'
  and proc_name like '%元%'
--   and regexp_extract(proc_name, '(\\d+)元', 1) >= 99
