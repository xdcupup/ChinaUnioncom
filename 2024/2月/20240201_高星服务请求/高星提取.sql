...
with t1 as (select service_type_name, star_level, dt_id, bus_pro_id, service_no, meaning
            ...
            where service_type_name in
                  ('服务请求>>咨询>>附加产品/服务>>星级查询渠道',
                   '服务请求>>咨询>>附加产品/服务>>星级查询渠道',
                   '服务请求>>咨询>>附加产品/服务>>星级查询渠道')
              and (dt_id >= '20240110' and dt_id <= '20240130')),
...
with t1 as (select *
     ...
     t2 as (select distinct device_number, vip_class_id
            ...
            where vip_class_id in ('600', '700')
              ...
              and day_id >= '19'
              and day_id <= '23'),
     t3 as (select serv_type_name, device_number, vip_class_id, busno_prov_name
            from t1
                     left join t2 on t1.busi_no = t2.device_number)
select '星级查询渠道咨询工单量-高星' as kpi_name, meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.busno_prov_name = bb.meaning
where region_code is not null
group by meaning
union all
select '星级查询渠道咨询工单量-高星' as kpi_name, '全国' as meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.busno_prov_name = bb.meaning
where region_code is not null;
...
with t1 as (select *
     ...
     t3 as (select serv_type_name, device_number, vip_class_id, busno_prov_name
            from t1
                     left join t2 on t1.busi_no = t2.device_number)
select '星级查询投诉量-高星' as kpi_name, meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.busno_prov_name = bb.meaning
where region_code is not null
group by meaning
union all
select '星级查询投诉量-高星' as kpi_name, '全国' as meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.busno_prov_name = bb.meaning
where region_code is not null;
...
-- '服务请求>>咨询>>附加产品/服务>>星级服务内容'
---- 高星
with t1 as (select service_type_name, dt_id, bus_pro_id, service_no
            from dc_dm.dm_d_callin_sheet_contact_agent_report aa
            where service_type_name in
                  ('服务请求>>咨询>>附加产品/服务>>星级服务内容',
                   '服务请求>>咨询>>附加产品/服务>>星级服务内容',
...
with t1 as (select service_type_name, star_level, dt_id, bus_pro_id, service_no
     ...
     t2 as (select distinct device_number, vip_class_id
            ...
            where vip_class_id in ('600', '700')
              ...
              and day_id >= '19'
              and day_id <= '30'),
     t3 as (select service_no, vip_class_id, device_number, bus_pro_id
            from t1
                     left join t2 on service_no = device_number)
select '星级查询渠道服务请求量-咨询-高星' as kpi_name, meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.bus_pro_id = bb.code
where region_code is not null
group by meaning
union all
select '星级查询渠道服务请求量-咨询-高星' as kpi_name, '全国' as meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.bus_pro_id = bb.code
where region_code is not null
;
...
where dt_id = '20240131'
limit 10;
show partitions db_der_arm.dwd_d_cus_e_cust_group_starinfo_tm;
-- todo 星级变动提醒服务请求量-咨询-高星
...
with t1 as (select service_type_name, star_level, dt_id, bus_pro_id, service_no
     ...
     t3 as (select service_no, vip_class_id, device_number, bus_pro_id
            from t1
                     left join t2 on service_no = device_number)
select '星级变动提醒服务请求量-咨询-高星' as kpi_name, meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.bus_pro_id = bb.code
where region_code is not null
group by meaning
union all
select '星级变动提醒服务请求量-咨询-高星' as kpi_name, '全国' as meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.bus_pro_id = bb.code
where region_code is not null
;
...
with t1 as (select busno_prov_name,
     ...
     t3 as (select *
            from t1
                     left join t2 on t1.busi_no = t2.device_number)
select '星级服务产品投诉量-高星'                                          as kpi_name,
       meaning,
       count(device_number)                                               as cnt,
...
with t1 as (select *
     ...
     t3 as (select serv_type_name, device_number, vip_class_id, busno_prov_name
            from t1
                     left join t2 on t1.busi_no = t2.device_number)
select '星级告知提醒不及时投诉量-高星' as kpi_name, meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.busno_prov_name = bb.meaning
where region_code is not null
group by meaning
union all
select '星级告知提醒不及时投诉量-高星' as kpi_name, '全国' as meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.busno_prov_name = bb.meaning
where region_code is not null;
...
with t1 as (select *
     ...
     t3 as (select serv_type_name, device_number, vip_class_id, busno_prov_name
            from t1
                     left join t2 on t1.busi_no = t2.device_number)
select '星级评定不认可投诉量-高星' as kpi_name, meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.busno_prov_name = bb.meaning
where region_code is not null
group by meaning
union all
select '星级评定不认可投诉量-高星' as kpi_name, '全国' as meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.busno_prov_name = bb.meaning
where region_code is not null;


-- todo 星级评定规则服务请求量-咨询-高星
-- 服务请求中含关键字“星级评定规则”的合计量：
-- 服务请求>>咨询>>附加产品/服务>>星级评定规则
-- 服务请求>>咨询>>附加产品/服务>>星级评定规则
-- 服务请求>>咨询>>附加产品/服务>>星级评定规则
with t1 as (select service_type_name, star_level, dt_id, bus_pro_id, service_no
     ...
     t3 as (select service_no, vip_class_id, device_number, bus_pro_id
            from t1
                     left join t2 on service_no = device_number)
select '星级评定规则服务请求量-咨询-高星' as kpi_name, meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.bus_pro_id = bb.code
where region_code is not null
group by meaning
union all
select '星级评定规则服务请求量-咨询-高星' as kpi_name, '全国' as meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.bus_pro_id = bb.code
where region_code is not null
;
...
with t1 as (select *
     ...
     t3 as (select serv_type_name, device_number, vip_class_id, busno_prov_name
            from t1
                     left join t2 on t1.busi_no = t2.device_number)
select '星级评定规则咨询工单量-高星' as kpi_name, meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.busno_prov_name = bb.meaning
where region_code is not null
group by meaning
union all
select '星级评定规则咨询工单量-高星' as kpi_name, '全国' as meaning, count(device_number) as cnt
from t3
         right join dc_dim.dim_province_code bb on t3.busno_prov_name = bb.meaning
where region_code is not null;
...
with t1 as (select busno_prov_name,
            ...
            where (
                (month_id = '202401' and day_id >= '19') and
                (month_id = '202401' and day_id <= '303')
                )
              and serv_type_name in
                  ( '咨询工单（2021版）>>附加产品/服务>>星级服务内容')),
     t2 as (select distinct device_number, vip_class_id
     ...
     t3 as (select *
            from t1
                     left join t2 on t1.busi_no = t2.device_number)
select '星级服务内容咨询工单量-高星'                                          as kpi_name,
       meaning,
       count(device_number)                                               as cnt,
       sum(case when serv_content rlike 'APP便捷服务' then 1 else 0 end)  as `APP便捷服务`,
       sum(case when serv_content rlike '营业厅预约办' then 1 else 0 end) as `营业厅预约办`,
