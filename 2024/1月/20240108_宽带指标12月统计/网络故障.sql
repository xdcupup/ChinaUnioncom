...
----总体满意率测评
with t1 as (select province_code as HANDLE_PROVINCE_CODE,
...
select handle_province_code,
       ...
       handle_cities_code,
       scene,
       bad_one_scene + good_one_scene + center_one_scene                    as fenmu,
       bad_one_scene,
       center_one_scene,
       good_one_scene,
       good_one_scene / (bad_one_scene + good_one_scene + center_one_scene) as rate
from t1
         left join dc_dim.dim_province_code aa on substr(t1.HANDLE_PROVINCE_CODE, 2, 3) = aa.code;


select province_code as HANDLE_PROVINCE_CODE,
       '00'          as HANDLE_CITIES_CODE,
       '3'              scene,
       count(1),
       sum(
               case
                   when locate("网络故障", answer_3) > 0
                       or locate("网络故障", answer_4) > 0
                       or locate("网络故障", answer_2) > 0 then answer_1
                   else 0
                   end
       )                total_source,
       sum(
               case
                   when answer_1 >= 1
                       and answer_1 <= 6
                       and locate("网络故障", answer_2) > 0 then 1
                   else 0
                   end
       )                bad_one_scene,
       sum(
               case
                   when answer_1 >= 7
                       and answer_1 <= 8
                       and (
                            locate("网络故障", answer_3) > 0
                                or locate("网络故障", answer_2) > 0
                            ) then 1
                   else 0
                   end
       )                center_one_scene,
