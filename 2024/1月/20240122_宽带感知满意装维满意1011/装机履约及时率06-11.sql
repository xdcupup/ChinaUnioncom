with t1 as (select *, row_number() over (partition by zj_order order by fitst_time desc) as rn
     ...
     t2 as (select *
            from t1
            where rn = 1
              and (hour(from_unixtime(unix_timestamp(book_time, 'yyyyMMddHHmmss'))) <= 17
                or (hour(from_unixtime(unix_timestamp(book_time, 'yyyyMMddHHmmss'))) >= 8 and
                    minute(from_unixtime(unix_timestamp(book_time, 'yyyyMMddHHmmss'))) < 30))),
     t3 as (select province_code,
            ...
            from t2
                     right join
                 dc_dwd.dpa_trade_all_dist_temp_202306_202311 a on a.trade_id = t2.zj_order
