...
---------------------------------------------8:30 - 17:00   受理
select busno_prov_id, busno_prov_name, kpi_name, fenzi, fenmu, Late_arrival, iom_null,fenmu, fenzi + Late_arrival + iom_null as compare
from (select busno_prov_id,
             busno_prov_name,
             '修障联系及时率'    as kpi_name,
             sum(case
                     when round((unix_timestamp(fitst_time) - unix_timestamp(accept_time, 'yyyy-MM-dd HH:mm:ss')) / 60,
                                2) <=
                          30 then 1
                     else 0 end) as fenzi,
             ...
             sum(case
                     when round((unix_timestamp(fitst_time) - unix_timestamp(accept_time, 'yyyy-MM-dd HH:mm:ss')) / 60,
                                2) >
                          30 or ((fitst_time is null or fitst_time = '' or accept_time is null or
                                  accept_time = '') and (kf_sn is not null or kf_sn != '')) then 1
                     else 0 end) as Late_arrival,
             sum(case
                     when kf_sn is null or kf_sn = '' then 1
                     else 0 end) as iom_null --客服工单流水号 或 首次联系用户时间 为空值或null的
      from (select *, row_number() over (partition by sheet_no order by fitst_time) as rn
            from (select sheet_no, sheet_id, accept_time, busno_prov_name, busno_prov_id
                  ...
                  where a.month_id = '202312'
                    ...
                    and a.serv_type_name in (
                     ...
                     left join(select xz_order as  kf_sn, arrive_time, book_time,code as  prov_id, province, fitst_time
                               from dc_dwd.omi_zhijia_time_2023_06_11
                               where month_id = '202312') t2
