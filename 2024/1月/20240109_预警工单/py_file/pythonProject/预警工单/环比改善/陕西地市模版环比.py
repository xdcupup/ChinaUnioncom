...
# 查询语句1 预警规则
query1 = f"select 'T月进行预警，围绕T-1月省分指标完成情况，预警规则定义为：两项指标均≤省分T-1月完成值视为需预警人员，如其中一项或N项已达标，按照达标目标口径制定。' as 预警原则,'{zj_value}'  as '{sf}2月省分装机当日通完成值','{xz_value}' as '{sf}2月省分修障当日好完成值','地市智家人员统计情况同时满足装机当日通率≤T-1月目标值*60%且修障当日好率≤T-1月目标值*60%' as 本月预警规则;"
...
# 查询语句6 装机当日通明细
query6 = f"select distinct t2.* from (select * from {no_standars_value} where 市 = '{area}' or 市 = '{area2}') t1  join {zj_mingxi} t2 on t1.工号 = t2.工号;"
df6 = pd.read_sql(query6, connection)

# 查询语句7 修障当日好明细
query7 = f"select distinct t2.* from (select * from {no_standars_value} where 市 = '{area}' or 市 = '{area2}') t1 join {xz_mingxi} t2 on t1.工号 = t2.工号;"
df7 = pd.read_sql(query7, connection)


# 创建一个Excel writer对象

writer = pd.ExcelWriter(f'C:/Users/14659/Desktop/预警工单数据/2月/{dt2}/{sf}/{area}地市{dt2}预警信息.xlsx')
