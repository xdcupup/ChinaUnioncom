...
select *
from db1.xzdrh_0101_0128_mingxi;
select * FROM db1.no_standards_0101_0128;
create table db1.no_standards_0101_0128
with t1 as (select 省份,
                   ...
                   指标值,
                   账期
            from db1.xzdrh_0101_0128_tongji),
     t2 as (select 省份,
                   市,
                   区,
                   班组,
                   ...
                   指标值,
                   账期
            from db1.zjdrt_0101_0128_tongji)
select t1.省份,
       t1.市,
       t1.区,
       t1.班组,
...
-- 创建 统计总表上周
drop table db1.all_0101_0128;
create table db1.all_0101_0128
with t1 as (select 省份,
                   市,
                   区,
                   班组,
...
with aa as (select distinct t1.省份,
            ...
            from db1.no_standards_0108_0114 t1
                     left join db1.all_0101_0128 t2 on t1.工号 = t2.工号)
select distinct 省份,
                市,
                区,
                班组,
...
select *
from db1.no_standards_0101_0128
where 省份 = '陕西';
...
alter table db1.zjdrt_0101_0128_mingxi
...
select distinct t2.cbss流水编号id,
                ...
                t2.`是否当日通 1：是 0：否`,
                t2.账期
from (select * from db1.no_standards_0101_0128 where 市 = '孝感市' or 市 = '孝感') t1
         join db1.zjdrt_0101_0128_mingxi t2 on t1.工号 = t2.工号;